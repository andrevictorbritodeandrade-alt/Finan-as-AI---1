...
const handleFirestoreError = (error: unknown, operationType: OperationType, path: string | null) => {
  ...
  // CRITICAL: Always throw the JSON error for systemic diagnosis
...
const App: React.FC = () => {
    ...
    // Force refresh to pull updated categories and grouping (v39) - Restore June salary and move Iago expenses
    useEffect(() => {
        const forceUpdateV39 = localStorage.getItem('force_update_v39_fix_jun');
        if (!forceUpdateV39) {
            ...
            localStorage.setItem('force_update_v39_fix_jun', 'true');
    ...
    const ensureSystemIntegrity = (data: MonthData, year: number, month: number): MonthData => {
        ...
        if (year === 2026 && month === 5) {
            ...
            // No longer forcing Lili here to avoid June being 3/5. financeUtils handles it.
    ...
    const saveData = async (data: MonthData | null, year: number, month: number) => {
        ...
        if (isConfigured && auth?.currentUser) {
            ...
            try {
                const docRef = doc(db, 'families', FAMILY_ID, 'months', `${year}_${month}`);
                // Use serverTimestamp for absolute temporal integrity as per Hardened Rules
                await setDoc(docRef, {
                    ...updatedData,
