...
export const generateMonthData = (year: number, month: number): MonthData => {
    ...
    const isJune2026OrLater = (year === 2026 && month >= 6) || (year > 2026);

    // Base Incomes
    let newIncomes: Transaction[] = [
        { id: `inc_m_${year}_${month}`, description: `SALARIO MARCELLY`, amount: 3436.22, paid: (isJan2026 || isApr2026OrMay2026) && !isJune2026OrLater, date: salaryDate, dueDate: salaryDate, category: 'Salário' },
        { id: `inc_a_${year}_${month}`, description: `SALARIO ANDRE`, amount: 3436.22, paid: (isJan2026 || isApr2026OrMay2026) && !isJune2026OrLater, date: salaryDate, dueDate: salaryDate, category: 'Salário' },
        { id: `inc_mum_m_${year}_${month}`, description: 'MUMBUCA MARCELLY', amount: 598.00, paid: (isJan2026 || isApr2026OrMay2026) && !isJune2026OrLater, date: mumbucaDate, category: 'Mumbuca' },
        { id: `inc_mum_a_${year}_${month}`, description: 'MUMBUCA ANDRE', amount: 598.00, paid: (isJan2026 || isApr2026OrMay2026) && !isJune2026OrLater, date: mumbucaDate, category: 'Mumbuca' }
    ];

    // André fired in June 2026
    if (year === 2026 && month === 6) {
        newIncomes = newIncomes.filter(inc => inc.description !== "SALARIO ANDRE");
        newIncomes.push(
            { id: `inc_a_13rd_${year}_${month}`, description: `13º PROPORCIONAL (4/12) - ANDRÉ`, amount: 1145.40, paid: false, date: salaryDate, dueDate: salaryDate, category: 'Salário' },
            { id: `inc_a_vac_${year}_${month}`, description: `FÉRIAS PROP. + 1/3 (10/12) - ANDRÉ`, amount: 3818.01, paid: false, date: salaryDate, dueDate: salaryDate, category: 'Salário' }
        );
    } else if (isJune2026OrLater) {
        // Remove standard salary from July 2026 onwards
        newIncomes = newIncomes.filter(inc => inc.description !== "SALARIO ANDRE");
    }
    ...
    const cyclicalConfig = [
        ...
        { description: "CARTÃO DO ITAÚ DO ANDRÉ", amount: isJune2026OrLater ? 100.00 : 116.00, category: "Moradia", day: 24, group: 'MORADIA' },
    ...
    cyclicalConfig.forEach(c => {
        ...
        if (isFeb2026 && (c.description.includes("CLARO ANDRÉ"))) {
            return; 
        }
        ...
        newExpenses.push({
            ...
            amount: finalAmount,
            category: c.category,
            paid: isPaid || (finalAmount === 0 && (month >= 4) && !isJune2026OrLater), // Auto-pay zeroed items only before June
    ...
    // Special Iago Expenses for June 2026 (moved from Avulsos)
    if (year === 2026 && month === 6) {
        newExpenses.push(
    ...
    const finiteConfig = [
        ...
        { 
          desc: "EMPRÉSTIMO COM LILI", 
          totalAmount: 4000.00, 
          cat: "Dívidas", day: 4, 
          installments: 5, 
          sY: 2026, 
          sM: isJune2026OrLater ? 5 : 4, // Shift to 5 so June is 2/5 ( (6-5)+1 = 2 )
          group: 'LILI TORRES' 
        },
        ...
        { desc: "CABESOM", totalAmount: 179.00, cat: "Dívidas", day: 28, installments: 2, sY: 2026, sM: 5, group: 'MARCIA BRITO' },
    ...
    finiteConfig.forEach(f => {
        // Special logic for Lili Torres: Skip May installment as she requested to pay on May 22 (June cycle)
        if (f.desc.includes("EMPRÉSTIMO COM LILI") && year === 2026 && month === 5) {
            return;
        }
        ...
        if (inst) {
            ...
            newExpenses.push({
                id: `fin_${f.desc.replace(/\s/g,'')}_${inst.current}_${year}_${month}`,
                description: f.desc, 
    ...
    if (year === 2026 && month === 5) {
        newAvulsosItems.push(
            ...
            { id: `avulso_iago_fuel_may`, description: "ABASTECIMENTO IAGO (04/05)", amount: 335.50, category: "Transporte", paid: true, dueDate: "2026-05-04", date: "2026-05-04", group: "IAGO" },
            { id: `avulso_iago_cel_may`, description: "CLAROFLEX ANDRE MARCELLY (04/05)", amount: 34.90, category: "Moradia", paid: true, dueDate: "2026-05-04", date: "2026-05-04", group: "IAGO" }
        );
    }
